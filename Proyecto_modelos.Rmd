...
hungria <- hungria %>% select(id, colnames(hungria[, 1:ncol(hungria)]))
suiza <- suiza %>% suiza(id, colnames(suiza[, 1:ncol(suiza)]))

# Se divide la base de EEUU en train y test
...
modelo.eeuu <- glm(num ~ age + sex, family = binomial, data = eeuu, na.action = na.omit)
modelo.hungria <- glm(num ~ age + sex, family = binomial, data = hungria, na.action = na.omit)
modelo.suiza <- glm(num ~ age + sex, family = binomial, data = suiza, na.action = na.omit)
```
