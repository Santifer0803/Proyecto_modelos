...
modelo <- train(
  ...
  data = eeuu,
  method = "rf",
  na.action = na.omit  # Opcional: omite filas con NA
)
```
...
ggplot(importancia, aes(x = reorder(Variable, Overall), y = Overall)) +
  ...
  labs(x = "Variable", y = "Importancia", title = "Importancia de Variables") +
  theme_minimal()
...
dimnames(TD1) <- list(
  "Enfermedad" = c("Sano", "Enfermo"),  # Filas (num)
  "Sexo" = c("Mujer", "Hombre")         # Columnas (sex)
)
...
mosaicplot(TD1, color = TRUE, main = 'Gráfico de Mosaico')
```
```{r}
mosaicplot(TD1,
           color = c("#66C2A5", "#FC8D62"),  # Colores personalizados
           main = "Prevalencia de Enfermedad Cardíaca por Sexo",
...
El siguiente gráfico tiene la finalidad de presentar los datos de manera proporcional, presentando siempre la misma información.

```{r}
ggplot(as.data.frame(TD1), aes(x = Sexo, y = Freq, fill = Enfermedad)) +
  geom_col(position = "fill") +  # Proporciones en lugar de conteos
  scale_fill_manual(values = c("Sano" = "#66C2A5", "Enfermo" = "#FC8D62")) +
...
# Si p-value es menor que 0.05 entonces se debe rechazar.
# En este caso el p-value es muy pequeño, por ende, se debe rechazar H0 e indicar que existe una relación entre la variable categórica sexo y la variable predictora.
mosaicplot(TD2, color = TRUE, main = 'Gráfico de Mosaico')
```
...
Se procede a calcular el Odd Ratio, con el fin de verificar la magnitud de esta asociación. 
...
Se gráfica utilizando proporción.
```{r}
ggplot(as.data.frame(TD2), aes(x = Sexo, y = Freq, fill = Enfermedad)) +
  geom_col(position = "fill") +  # Proporciones en lugar de conteos
  scale_fill_manual(values = c("Sano" = "#66C2A5", "Enfermo" = "#FC8D62")) +
  labs(title = "Proporción de Enfermedad Cardíaca por Sexo",
       y = "Proporción",
  ...
  theme_minimal()
```
Seguidamente, la variable analizar será ```cp```, la cual hace referencia a la Angina de pecho, para ello, se convierte la variable a factor, es decir a una variable categórica, con el fin de que se realice un procedimiento análogo al anterior. 
...
dimnames(TD3) <- list(
  ...
  "Tipo de Angina (Dolor de Pecho)" = c("Angina Típica", "Angina Atípica", "Dolor sin Angina", "Asintomático")         
)
...
# Si p-value es menor que 0.05 entonces se debe rechazar.
# En este caso el p-value es muy pequeño, por ende, se debe rechazar H0 e indicar que existe una relación entre la variable categórica sexo y la variable predictora.
mosaicplot(TD3, color = TRUE, main = 'Gráfico de Mosaico')
```
...
# Si p-value es menor que 0.05 entonces se debe rechazar.
# En este caso el p-value es muy pequeño, por ende, se debe rechazar H0 e indicar que existe una relación entre la variable categórica sexo y la variable predictora.
mosaicplot(TD4, color = c("#66c2a5", "#fc8d62", "#8da0cb", "gray80"), main = 'Gráfico de Mosaico', las = 1)
```
...
# Si p-value es menor que 0.05 entonces se debe rechazar.
# En este caso el p-value es muy pequeño, por ende, se debe rechazar H0 e indicar que existe una relación entre la variable categórica sexo y la variable predictora.
mosaicplot(TD5, color = c("#66c2a5", "#fc8d62", "#8da0cb"), main = 'Gráfico de Mosaico', las = 1)
```
...
print(TD6)
chisq.test(TD6)
mosaicplot(TD6, color = TRUE, main = 'Gráfico de Mosaico', las = 1)
```
...
print(TD7)
chisq.test(TD7)
mosaicplot(TD7, color = TRUE, main = 'Gráfico de Mosaico', las = 1)
```
...
print(TD8)
chisq.test(TD8)
mosaicplot(TD8, color = TRUE, main = 'Gráfico de Mosaico', las = 1)
```

Se procede con el cálculo del Odd Ratio perteneciente a Europa, para evaluar la magnitud. 
...
OR_angina_europa
```
Para el caso de Europa el odd ratio sugiere que las personas que presentaron angina inducida por ejercicio tienen 11 veces más probabilidad de presentar la enfermedad de las personas que no presentaron dicho dolor. 
...
print(TD9)
chisq.test(TD9)
mosaicplot(TD9, color = c("#66c2a5", "#fc8d62", "#8da0cb"), main = 'Gráfico de Mosaico', las = 1)
```
...
print(TD10)
chisq.test(TD10)
mosaicplot(TD10, color = c("#66c2a5", "#fc8d62", "#8da0cb"), main = 'Gráfico de Mosaico', las = 1)
```
Para el caso de Europa, se tiene el problema de poca muestra en el conteo de lagunas celdas de la tabla de la tabla de contingencia, lo que puede provocar un error en los cálculos, al observar el gráfico de mosaico se observa una proporción más o menos igual entre los segmentos constante y decreciente. En el caso de ser creciente, hay una diferencia relativamente significativa, lo que puede sugerir que para este caso hay una relación. Al aplicar la prueba de hipótesis, se tiene que el p-value es inferior al 5%, lo que indica que se debe rechazar la hipótesis nula, sin embargo esta evidencia es estadísticamente débil.
...
print(TD11)
chisq.test(TD11)
mosaicplot(TD11, color = c("#66c2a5", "#fc8d62", "#8da0cb"), main = 'Gráfico de Mosaico', las = 1)
```
...
print(TD12)
chisq.test(TD12)
mosaicplot(TD12, color = c("#66c2a5", "#fc8d62", "#8da0cb"), main = 'Gráfico de Mosaico', las = 1)
```
